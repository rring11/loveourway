...
{/metadocument}
*/
	var $configuration_file = 'libs/oauth-api-2016-05-11/oauth_configuration.json';
...
*/
	...
	Function SetupSession(&$session)
	{
		...
		// POSSIBLY ADD LOGIC TO FIND SESSION BASED ON USER AND SERVER?
		// WOULD NEED TO MAKE SURE USER AND SERVER IS SET PRIOR TO THIS CALL!
...
//		$session->session = md5($session->state.time().rand());
        $session->session = $_SESSION['user_id'];
	...
	Function Initialize()
	{
		...
		switch($this->server)
		{
			case 'Pinterest':
				...
				$this->dialog_url = 'https://api.pinterest.com/oauth?client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI}&scope={SCOPE}&state={STATE}&response_type=code';
				$this->reauthenticate_dialog_url = 'https://api.pinterest.com/oauth?client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI}&scope={SCOPE}&state={STATE}&auth_type=reauthenticate';
