...
class database_oauth_client_class extends oauth_client_class
{
	...
	var $session = '';
	var $user = 0;
	var $session_cookie = 'social_media_sessions';
	var $session_path = '/';
	var $sessions = array();
	...
	Function CreateOAuthSession($user, &$session)
	{
		...
		if(!$this->Query('INSERT INTO social_media_sessions (session, state, access_token, access_token_secret, expiry, authorized, type, server, creation, refresh_token, access_token_response) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', $parameters, $results))
			return false;
	...
	Function GetUserSession($user, &$oauth_session)
	{
		...
		if(!$this->Query('SELECT id, session, state, access_token, access_token_secret, expiry, authorized, type, server, creation, refresh_token, access_token_response FROM social_media_sessions WHERE user=? AND server=?', $parameters, $results, $result_types))
			return false;
	...
	Function GetOAuthSession($session, $server, &$oauth_session)
	{
		...
		$result_types = array(   'i','s',     's',   's',          's',                 'ts',   'b',        's',  's',    'ts',     's',           's');
		if(!$this->Query('SELECT id, session, state, access_token, access_token_secret, expiry, authorized, type, server, creation, refresh_token, access_token_response FROM social_media_sessions WHERE session=? AND server=?', $parameters, $results, $result_types))
			return false;
		if(count($results['rows']) === 0)
		{
			$oauth_session = null;
	...
	Function StoreAccessToken($access_token)
	{
		...
		return $this->Query('UPDATE social_media_sessions SET session=?, state=?, access_token=?, access_token_secret=?, expiry=?, authorized=?, type=?, server=?, creation=?, refresh_token=?, access_token_response=?, user=? WHERE id=?', $parameters, $results);
	}
	...
	Function SetUser($user)
	{
		...
		if(!$this->Query('UPDATE social_media_sessions SET user=? WHERE session=? AND server=?', $parameters, $results))
			return false;
		$this->user = $user;
		return true;
	}
